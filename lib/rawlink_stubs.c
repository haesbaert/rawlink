 ...
 */
int bpf_setpromisc(int fd, value vpromisc) {
  int r;
...
int bpf_setfilter(int fd, value vfilter) {
  ...
  if (vfilter == Val_int(0))
    return (0);
  prog.bf_len = caml_string_length(Field(vfilter, 0)) / sizeof(struct bpf_insn);
...
CAMLprim value caml_rawlink_open(value vfilter, value vpromisc, value vifname) {
...
CAMLprim value caml_bpf_align(value va, value vb) {
  CAMLparam2(va, vb);
  CAMLlocal1(v);
  uint32_t a, b;
...
int af_packet_setfilter(int fd, value vfilter) {
  ...
  prog.len = caml_string_length(Field(vfilter, 0)) / sizeof(struct sock_filter);
...
int af_packet_setpromisc(int fd, int promisc, int setclr) {
  int r;
  struct ifreq ifreq;
...
CAMLprim value caml_rawlink_open(value vfilter, value vpromisc, value vifname) {
  ...
  if (af_packet_setfilter(fd, vfilter) == -1)
    CAMLreturn(Val_unit);
  if (af_packet_setpromisc(fd, Bool_val(vpromisc), 1) == -1)
    CAMLreturn(Val_unit);
...
CAMLprim value caml_rawlink_close(value vfd, value vpromisc) {
  CAMLparam2(vfd, vpromisc);
  int r;
...
CAMLprim value caml_driver(value vunit) {
  CAMLparam0();
#ifdef AF_PACKET
  CAMLreturn(Val_int(0));
#else
...
/* Filters */
CAMLprim value caml_dhcp_server_filter(value vunit) {
  CAMLparam0();
  CAMLlocal1(vfilter);
...
CAMLprim value caml_dhcp_client_filter(value vunit) {
  CAMLparam0();
  CAMLlocal1(vfilter);
  struct FILTER dhcp_bpf_filter[] = {
      /* Make sure this is an IP packet... */
